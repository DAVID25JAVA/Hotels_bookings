...
function GuestRating() {
  return (
    <div className="bg-slate-50 md:py-10 py-5">
      ...
      <div className="flex flex-wrap items-center justify-center gap-20 md:gap-6 md:py-24 py-20">
        {testimonials?.map((item, index) => (
          <div
            key={index}
            className="text-sm w-80 md:h-56 border border-gray-500/30 pb-6 rounded-lg bg-white shadow-[0px_4px_15px_0px] shadow-black/5"
          >
            <div className="flex flex-col items-center px-5 py-4 relative">
              <img
                className="h-20 w-20 absolute -top-11 rounded-full"
                src={item?.image}
                alt="userImage1"
              />
            ...
            <p className="text-gray-500 px-6 text-[15px] text-center">
              {item?.review}
            </p>
